...
export class ProfileController {
  ...
  @Get('search')
  @ApiOperation({ summary: 'Searches for people given name and/or email and returns their profile info' })
  ...
  @ApiBadRequestResponse( { description: `Bad Request (${PROFILE_SEARCH_BAD_REQUEST})`} )
  //@ApiUnauthorizedResponse({ description: 'Unauthorized'} )
  @ApiInternalServerErrorResponse( { description: 'Unexpected Error'} )
  getProfilesBySearch(@Query('email') email: string, @Query('name') name: string): Promise<Person[]>{
    return this.profileService.profilesBySearch({ email, name });
  }

  @Get(':id')
  @ApiOperation({ summary: 'Returns profile info for given id including shoutouts given and received' })
  //@ApiUnauthorizedResponse({ description: 'Unauthorized'} )
