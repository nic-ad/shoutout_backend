...
describe("profile search by name/email", function(){
  it("should find only profiles whose name contains the search query", async function(){
    ...
    const response = await request(app).get(`/profile/search?name=${searchQuery}`);
    const results = response.body;
    const resultWithoutQueryInName = results.find(profile => profile.name.toLowerCase().indexOf(searchQuery.toLowerCase()) < 0);
  ...
  it("should find only profiles whose email contains the search query", async function(){
    ...
    const response = await request(app).get(`/profile/search?email=${searchQuery}`);
    const results = response.body;
    const resultWithoutQueryInEmail = results.find(profile => profile.email.indexOf(searchQuery.toLowerCase()) < 0);
  ...
  it("should return no results given garbage search query", async function(){
    const response = await request(app).get("/profile/search?name=alakjdsnlasnljkanlksnadlnaksnda");
    const results = response.body;
  ...
  it("should have same number of results from name + email search as when each searched separately", async function(){
    ...
    const togetherResults = await request(app).get(`/profile/search?name=${searchQuery}&email=${searchQuery}`);
    
    const nameResults = await request(app).get(`/profile/search?name=${searchQuery}`);
    const emailResults = await request(app).get(`/profile/search?email=${searchQuery}`);

    const namePlusEmailResults = [...nameResults.body, ...emailResults.body];
    const distinctResults = [...new Set(namePlusEmailResults.map(result => result._id))];
...
async function expectShoutoutReceived(shoutoutTimestamp, recipientId){
  ...
  const testShoutoutsReceived = result.shoutoutsReceived.filter(shoutout => shoutout.text.indexOf(shoutoutTimestamp) > -1);
...
describe("single-recipient shoutout on user profiles", function(){
  ...
  it("author profile should have shoutout given with self as author", async function(){
    const response = await request(app).get(`/profile/${singleRecipientShoutout.authorId}`);
    const result = response.body;
    const testShoutoutsGiven = result.shoutoutsGiven.filter(shoutout => shoutout.text.indexOf(shoutoutTimestamp) > -1);

    expect(response.status).to.equal(200);
    expect(testShoutoutsGiven.length).to.equal(1);
    expect(testShoutoutsGiven[0].author._id).to.equal(singleRecipientShoutout.authorId);
  });
